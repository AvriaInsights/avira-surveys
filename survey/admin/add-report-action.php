...
$uploadinfographics = "infographics";
...
if ($_POST['url_keywords'] == NULL) {
    $_SESSION['error'] = "Please enter the URL Keywords";
    header("Location:" . $_SERVER['HTTP_REFERER']);
}
...
$url_keywords = $obj_report->cleanOutput($_POST['url_keywords']);
...
if ($_POST['update_type'] == "add")
{
    ...
    $keywords = $obj_report->removeSpace($_POST['url_keywords']);
    $keywords_slug = $obj_report->removeSpace($_POST['url_keywords']."-Global");
                    
    $slug = $obj_report->removeSpace($_POST['url_keywords']."");
    ...
    if (isset($_FILES['sample']['name']) && $_FILES['sample']['error'] == 0) {
      ...
      //Infographics PDF
    if (isset($_FILES['infographics']['name']) && $_FILES['infographics']['error'] == 0) {
        $file_tmp_name = $_FILES['infographics']['tmp_name'];
        $path_parts = pathinfo($_FILES['infographics']['name']);
        $extension = $path_parts['extension'];
        if ($extension == "pdf") {
            $file_name = "infographics" . uniqid() . ".$extension";
            if(!is_dir($uploadinfographics)){
                mkdir($uploadinfographics, 0777);
            }
            $file_path = $uploadinfographics . '/' . $file_name;
            $result1 = move_uploaded_file($file_tmp_name, $file_path);
           
            $insert_data['infographics'] = $file_path;
        } 
    } 
    ...
    $keywords = $obj_report->removeSpace($url_keywords);
    $keywords_slug = $obj_report->removeSpace($url_keywords."-Global");
    ...
    if (isset($_FILES['infographics']['name']) && $_FILES['infographics']['error'] == 0) {
        ...
        if ($extension == "pdf") {
            ...
            $update_data['infographics'] = $file_path;
        } 
    } 
    ...
    $update_data['is_bulk'] = mysqli_real_escape_string($conn, "NULL");
    $update_data['featured'] = "Yes";
