...
{
    $condition .=" tbl_discount.created_at>='" .$from_date ."' AND  tbl_discount.created_at<='" .$to_date."'";
}

$report_details = $obj_report->getDiscountDetails($fields, $condition, '`tbl_discount`.`discount_id` ASC', '', 0);

if (isset($report_details) && !empty($report_details)) {
    $delimiter = ",";
    $filename = "Request-Discount-" . date('M j Y h:i A') . ".csv";
   ...
   /* if($report_details)  
    ...
    foreach ($report_details as $report_detail) {
        ...
        $url = SITEPATH . 'report/' . $report_detail['report_id'] . '/' . "discount";
