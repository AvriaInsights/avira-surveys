...
//   $condition_survey_result = "`tbl_response_user`.`user_id` = '".$userid."' and (`tbl_survey`.`survey_title` like '%".$searchval."%' or `tbl_survey`.`survey_title` like '%".$searchval."' or `tbl_survey`.`survey_title` like '".$searchval."%')";
// }
// else
// {
...
{
   $searchval=$_GET['searchval'];
   $condition_survey_result = "`tbl_survey`.`user_id` = '".$userid."' and `tbl_survey`.`survey_fill_at`!='' and (`tbl_survey`.`survey_title` like '%".$searchval."%' or `tbl_survey`.`survey_title` like '%".$searchval."' or `tbl_survey`.`survey_title` like '".$searchval."%' or `tbl_survey`.`campaign_name` like '%".$searchval."%' or `tbl_survey`.`campaign_name` like '%".$searchval."' or `tbl_survey`.`campaign_name` like '".$searchval."%')";
}
else
{
    $condition_survey_result = "`tbl_survey`.`user_id` = '".$userid."' and `tbl_survey`.`survey_fill_at`!=''";
}
/**********All Active Survey***************/
$fields_survey_result = "`tbl_survey`.`survey_id`,`tbl_survey`.`survey_title`,`tbl_survey`.`survey_fill_at`";
...
$all_result_surveys=$obj_survey->getSurveyDetail($fields_survey_result, $condition_survey_result, $orderby_survey, $limit,'', 0);
$all_result_surveys_cnt=$obj_survey->getSurveyDetail($fields_survey_result, $condition_survey_result, '', '', 0);
    ...
    foreach($all_result_surveys as $all_result_survey){
        //$total_bhcnt="";
          /**********All Active Survey***************/
            ...
            $all_bhcnt=$obj_survey->getSurveyUser('`tbl_response_user`.`response_user_id`', $condition_bhcnt, '', '', 0);
