...
$fields = "response_user_id,user_fullname,user_email,ip_address,survey_fill_position,feedback_id,created_at";
$campaign_survey_details = $obj_survey->getSurveyUser($fields, $condition, $orderby,'', 0);

/**********Survey Details***********/
$fields_survey_details="survey_title";
$condition_survey="`tbl_survey`.`survey_id`='".$_GET['surveyid']."'";
$survey_details = $obj_survey->getSurveyDetail($fields_survey_details, $condition_survey, '','', 0);
...
$survey_url=SITEPATHFRONT."survey-view/".$surveyid."/".$survey_url;
/***********End Survey Details*********/
//die();
if (isset($campaign_survey_details) && !empty($campaign_survey_details)) {
    ...
    foreach ($campaign_survey_details as $campaign_survey_detail) {
        ...
        /*******Campaign Detail*********/
            $field_camp="q_last_attempt_qno,q_count";
            $condition_camp="`tbl_campaign_user`.`camp_response_user_id`='".$campaign_survey_detail['response_user_id']."'";
            $camps=$obj_survey->getCampaignSurveyDetails($field_camp, $condition_camp, '', '', 0);
            $camp=end($camps);
        ...
        /*******Feedback Detail********/
            $field_feedback="feed_data";
            $condition_fedbk="`tbl_feedback`.`feed_id`='".$campaign_survey_detail['feedback_id']."'";
            $feedbacks=$obj_survey->getFeedbackDetails($field_feedback, $condition_fedbk, '', '', 0);
