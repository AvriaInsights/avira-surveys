...
$all_active_users=$obj_survey_client->getSurveyUserDetail($fields_survey_client, $condition_user_inactive, $orderby, $limit,'', 0);
$all_active_users_cnt=$obj_survey_client->getSurveyUserDetail($fields_survey_client, $condition_user_inactive, $orderby, '', 0);
                    ...
                    ?>
                        ...
                        <li class="table-row sign_table_row">
                          ...
                          <div class="col col-7 text-center" data-label="Action">
                              <a class="btn btn-outline-danger" data-toggle="tooltip" data-placement="top" title="Ativate User" onclick="action1(<?php echo $all_active_user['client_id'];?>);">Activate</a>
                          </div>
                        </li>
                         <?php } ?>
                      </ul>  
...
<div class="clearfix">
	...
	if(!empty($total_pages)){
		for($i=1; $i<=$total_pages; $i++){
				if($i == 1){
					?>
				<li class="pageitem1 active" id="<?php echo $i;?>"><a href="JavaScript:Void(0);" data-id="<?php echo $i;?>" class="page-link1 active" ><?php echo $i;?></a></li>
											
				<?php 
				}
				else{
					?>
				<li class="pageitem1" id="<?php echo $i;?>"><a href="JavaScript:Void(0);" class="page-link1" data-id="<?php echo $i;?>"><?php echo $i;?></a></li>
				<?php
				}
		}
	}
...
<script type="text/javascript">
	...
	function action1(suseridactive)
    {
    //alert(userid);exit;
    
        $.ajax({
              url : "update-surveyuser-statusinactive.php",
              type : "POST",
              data : {suseridactive:suseridactive},
              success: function(){
