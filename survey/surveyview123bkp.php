                            ...
                            <div class="col-md-8 que_section offset-md-2" id="divQstn">
                             ...
                             ?>
                                       ...
                                       <div class="chk_class">
            				                     <div class="row pb-4 sub_points_down_chk" id="sub_scrollbar_diff">
            				                        <?php $counter = 0; foreach($all_subpoints as $all_subpoint){ $counter++; ?>
                                                     ...
                                                     <?php }  ?>
                                                     <input type="hidden" value="<?php echo $counter;?>" id="chk_sub_count" name="chk_sub_count">
                			   ...
                			   <?php if ($all_question['quest_type_id'] == "6") { ?> 
                                ...
                                <?php if ($all_question['quest_type_id'] == "8") { $spqstsubids="";?> 
                                  <div id="que_<?php echo $srno;?>" style="display:none" class="que_scroll item mt-1 mb-3" data-id='<?php echo $srno;?>'  isAttempted="0">
                                    ...
                                    ?>
                                          ...
                                          <div class="col-md-8">
                    							  <label class="for-checkbox-tools rank_label float-start" for="order_<?php echo $all_subpoint['question_subid'];?>">
                    							     ...
                    							     <span class="ml-3"><?php echo stripslashes($all_subpoint['question_subtitle']);?></span>
                    							  </label>
                                          </div>
                                          <div class="col-md-4">
                                                  <select class="rank_ord checkbox-tools1 float-end" id="order_<?php echo $all_subpoint['question_subid'];?>" name="order_<?php echo $all_subpoint['question_subid'];?>" onchange="checksequence(<?php echo $all_subpoint['question_subid'];?>,<?php echo $all_question['question_id'];?>);setorderquestans('<?php echo stripslashes(str_replace("'", "", str_replace('"', "", $all_subpoint["question_subtitle"])));?>',<?php echo $srno;?>,<?php echo $all_subpoint['question_subid'];?>,<?php echo $all_question['question_id'];?>,this.value);">
                    							 ...
                    							 <input type="hidden" id="sub_seq_id" name="sub_seq_id" value="<?php echo $all_subpoint['question_subid'];?>">
                                          </div>
                                         <?php } ?>
        				        	 ...
        				        	 <div class="col-md-12 mt-2 mb-4">
            				        	 <div class="button" id="button_next_order_<?php echo $all_question['question_id'];?>" onclick="checksequence(<?php echo $all_subpoint['question_subid'];?>,<?php echo $all_question['question_id'];?>);SetAttempted(this,<?php echo $all_question['sequence'];?>,<?php echo $all_question['question_id'];?>);setnext(<?php echo $srno;?>,<?php echo $all_subpoint['question_subid'];?>,<?php echo $all_question['question_id'];?>,'');">
                                             <div id="slide" class="slide"></div>
                                             <a class="action-btn-disabled save-next-btn" href="javascript:void(0);" id="btn_next_<?php echo $all_question['question_id'];?>">Save & Next &nbsp;&nbsp;<i class="fa fa-angle-right"></i></a>
                                        </div>
                                    </div>
...
$(document).ready(function(){
        var sub_count = $('#chk_sub_count').val();
        var rd_sub_point = $('#rdo_sub_count').val();
        var arr_count_rdo = $('#arr_count_rdo').val();
        var rank_sub_count = $('#rank_sub_count').val(); 
       
        if(sub_count >= 5){
        $('.sub_points_down_chk').css('height','200px');
        $('.sub_points_down_chk').css('overflow-y','scroll');
        }
        if(rd_sub_point >= 5)
        {
            
           $('.sub_points_down').css('height','250px');
           $('.sub_points_down').css('overflow-y','scroll'); 
        }
         if(rank_sub_count >= 4){
        $('.sub_points_rank').css('height','250px');
        $('.sub_points_rank').css('overflow-y','scroll');
        }
        
        /*for(var i=1;i<=arr_count_rdo;i++){
             var rdo_sub_char_count =$('#rdo_sub_char_count_'+i).val();
            
              if(rdo_sub_char_count >= 100)
            {
            
            $('.for_radio_label_charcount').css({"width":"770px"}); 
            $('.sub_points_down_chk').css({"height":"250px"});
            $('.sub_points_down_chk').css({"overflow-y":"scroll"});
            }
        }*/
});
            ...
            $("#lastsubmit").click(function(){
                ...
                if(fname_error_message === false && email_error_message === false)
                {
                    $.ajax({
        				...
        				type: "POST",
        				data: {allquestionids:allquestionids,allanswers:allanswers,fname:fname,email:email,surveyid:<?php echo $surveyid?>},
        				cache: false,
        				beforeSend: function(){
...
{  //alert(curqno);
   ...
   //var qseq=$("#qsequence").val();
        ...
        else
        {
            ...
            {
                 //var ordercnt=$("#rank_cnt_"+qid).val();
                //var orderarraylength = orderids.length;
                var ordercnt=$("#rank_cnt_"+qid).val();
                //var orderarraylength = orderids.length;
                var orderid11 =$("#orderselectedid_"+qid).val();
                //alert(orderid11);
                var orderid12=orderid11.split(";");
                var orderarraylength=orderid12.length;
                //alert(ordercnt);
                //alert(orderarraylength); 
               
                var selval=$("#orderselectedval_"+qid).val();
                $("#current_answer").val(selval);
...
function setorderquestans(title,curqno,subid,qid,ans)
{
    var allsubid = $("#allsubid_"+qid).val();
     var allsubidsplit = allsubid.split(",");
     var seqcnt=allsubidsplit.length;
    if($("#orderselectedid_"+qid).val()=="")
    {
         $("#orderselectedid_"+qid).val(subid);
         
    }
    else
    {
        var subids = $("#orderselectedid_"+qid).val();
        var subidsarray = subids.split(";");
       
        var nqid12 = subid+'';
        var posq12 = subidsarray.indexOf(nqid12);
        if(posq12=="-1")
        {
            $("#orderselectedid_"+qid).val(subids+";"+subid);
        }
        
    }
    //alert(subqidposition);
    if(ans!="")
    {
        var orderValue = title+"::"+ans;
    }
    else
    {
        var orderValue = "";
    ...
    //alert(orderValue);
    if($("#orderselectedval_"+qid).val()=="")
    {
          $("#orderselectedval_"+qid).val(orderValue);
          orderids.push(subid);
    }
    else
    {
          var prevans1 = $("#orderselectedval_"+qid).val();
          var ordervalcurrent=orderValue;
          var subid22 = subid+'';
          //alert(subid22);
          var orderid11 =$("#orderselectedid_"+qid).val();
          var orderid12=orderid11.split(";");
          //var orderarraylength=orderid12.length;
          var poschk = orderid12.indexOf(subid22);
          //alert(orderids);
          //alert(poschk);
          if(poschk=="-1")
          {
               orderids.push(subid);
               $("#orderselectedval_"+qid).val(prevans1+"*"+ordervalcurrent);
          }
          else
          {
              if(orderValue=="")
              {
                  ...
                  splitorder.splice(poschk, 1);
                  orderid12.splice(poschk, 1);
                  //alert(splitorder);
                  
                  ////////////////////
                  var newval = splitorder+'';
                  var test2211 = newval.replace(/,/g, '*');
                  $("#orderselectedval_"+qid).val(test2211);
                  
                  ///////////////////
                  var newval12 = orderid12+'';
                  var test221112 = newval12.replace(/,/g, ';');
                  $("#orderselectedid_"+qid).val(test221112);
              }
              else
              {
                  var splitorder = prevans1.split("*");
                  splitorder.splice(poschk, 1,orderValue);
              ...
              //alert(orderid12);
            ////////////////////Code for deselect rank order///////////////
            if(orderValue=="")
            {
                var allselval = $("#orderselectedval_"+qid).val();
                var allselvalsplit = allselval.split("*");
                if(allselvalsplit.length>0)
                {
                    for(var ttt=0;ttt<allselvalsplit.length;ttt++)
                    {
                        var singleval=allselvalsplit[ttt];
                        var singleselvalsplit = singleval.split("::");
                        var seq=singleselvalsplit[1];
                        subidarr.push(seq);
                ...
                for(var it=1;it<=seqcnt;it++)
                {
                        var ttstr=it+'';
                        var chkext=subidarr.indexOf(ttstr);
                ...
                for(var tk=0;tk<unchangarr.length;tk++)
                {
                    //alert(allsubidsplit);
                    var thisOptionValue=unchangarr[tk];
                    //alert(thisOptionValue);
                    for(var ts=0; ts<allsubidsplit.length; ts++)
                    {//alert(allsubidsplit[ts]);
                        
                          var opt = $("#order_"+allsubidsplit[ts]+" option[value='" + thisOptionValue + "']").prop('disabled', false); 
                    }
                }
                subidarr = [];
                unchangarr = [];
    ...
    var ordercnt=$("#rank_cnt_"+qid).val();
    //var orderarraylength = orderids.length;
    var errortxt=$('#seq_error_message_'+qid).html();
...
// $(document).ready(function(){
//       var order_id_seq = $("#sub_seq_id").val(); 
//     //alert(order_id_seq);
...
function checksequence(subqueid,qid){
   //alert(subtitle);
     ...
     //alert(seq);
        if(seq == '')
        {
            $('#seq_error_message_'+qid).show();
            $('#seq_error_message_'+qid).html("Select all Orders");
            $('#btn_next_'+qid).addClass("action-btn-disabled");
            
        }
        else
        {
            $('#seq_error_message_'+qid).hide();
            $('#seq_error_message_'+qid).html("");
            subidarr = [];
            unchangarr = [];
            var allsubid = $("#allsubid_"+qid).val();
            var allsubidsplit = allsubid.split(",");
            var thisOptionValue=$("#order_"+subqueid+" option:selected").val();
            
            
                var flag=0;
                if(setopt.length==0 && thisOptionValue!="")
                {
                    setopt.push(thisOptionValue);
                    for(var ttt=0; ttt<allsubidsplit.length; ttt++)
                    {
                           var opt = $("#order_"+allsubidsplit[ttt]+" option[value='" + thisOptionValue + "']"); 
                           opt.attr('disabled', 'true');
                           var posq123 = setoptselval.indexOf(thisOptionValue);
                            if(posq123=="-1")
                            {
                              setoptselval.push(thisOptionValue);
                            }
                ...
                {
                    var arrpop = setopt.pop();
                    //alert(setoptselval);
                    for(var ttt=0; ttt<allsubidsplit.length; ttt++)
                    {
                        
                        var leftoptionvalue=$("#order_"+allsubidsplit[ttt]+" option:selected").val();
                        //alert(leftoptionvalue);
                        //alert(arrpop);
                        // if(leftoptionvalue!="")
                        // {
                        //     setoptselval.push(leftoptionvalue);
                        // }
                         alert(setoptselval);
                         var posq1234 = setoptselval.indexOf(arrpop);
                         //alert(posq123);
                        //alert(flag);
                        //alert(setoptselval);
                        //if(posq1234!="-1")
                        //{
                        //alert(flag);
                            if(leftoptionvalue!=arrpop && flag==0)
                            {
                                
                                //alert(setoptselval);
                                // var posq12345 = setoptselval.indexOf(arrpop);
                                // if(posq12345=="-1")
                                // {
                                //     var opt1 = $("#order_"+allsubidsplit[ttt]+" option[value='" + arrpop + "']").prop('disabled', false); //enable
                                    
                                // }
                                // else
                                // {
                                //     var opt1 = $("#order_"+allsubidsplit[ttt]+" option[value='" + arrpop + "']").prop('disabled', true);
                                //     setoptselval.pop(arrpop);
                                
                                // }
                                //setoptselval.pop(arrpop);
                                for(var kk=0;kk<setoptselval.length;kk++)
                                {
                                    if(arrpop!=setoptselval[kk])
                                    {
                                    var opt1 = $("#order_"+allsubidsplit[ttt]+" option[value='" + setoptselval[kk] + "']").prop('disabled', true);
                                    }
                                    else
                                    {
                                      var opt1 = $("#order_"+allsubidsplit[ttt]+" option[value='" + setoptselval[kk] + "']").prop('disabled', false);  
                                    }
                                }
