...
{
    $condition_survey_result = "`tbl_response_user`.`status` = 'Active' and `tbl_survey`.`user_id` = '".$userid."' and `tbl_response_user`.`survey_id`='".$_GET['surveyid']."' and `tbl_response_user`.`survey_fill_position` = 'Half'";
}
else
{
    $condition_survey_result = "`tbl_response_user`.`status` = 'Active' and `tbl_survey`.`user_id` = '".$userid."' and `tbl_response_user`.`survey_fill_position`='Half'";
}
...
<style>
 ...
 <div class="row listingbox d-flex align-items-center">
        <?php if(isset($_GET['surveyid']) && !empty($_GET['surveyid'])){?>
         <?php if($all_result_survey['user_fullname'] =="" && $all_result_survey['user_email'] =="") { ?>
         <div class="col-md-4">
            <div class="row"><div class="col-md-12 txtbold"><?php echo $all_result_survey['ip_address'];?></div></div>
        </div>
        ...
        <div class="col-md-2">
            <div class="row"><div class="col-md-12 txtbold"><?php $updat=explode(" ",$all_result_survey['updated_at']);
            //echo $updat[0];
                ...
                <div class="col-md-12" style="padding:11px;">
                    <div class="row">
                        <div class="col-md-12 txtbold">
                            <!--<a href="<?php echo SITEPATH;?>add-survey?surveyid=<?php echo $all_active_survey['survey_id'];?>"></a>-->
                                <?php echo $all_result_survey['survey_title'];?>
                            </div>
                        </div>
                    <div class="row"><div class="col-md-12">Last Modified: <?php echo $all_result_survey['updated_at'];?></div></div>
                </div>
            </div>
        </div>
        <?php if($all_result_survey['user_fullname'] =="") { ?>
         <div class="col-md-2">
            <div class="row"><div class="col-md-12 txtbold"><?php echo $all_result_survey['ip_address'];?></div></div>
        ...
        <div class="col-md-2">
            <div class="row"><div class="col-md-12">
                <a href="#" data-bs-toggle="modal" data-bs-target="#result" onclick="setresponseid(<?php echo $all_result_survey['response_user_id'];?>,'<?php echo $all_result_survey['survey_id'];?>')" class="view-survey-btn">View Survey Result</a>
            </div></div>
        </div>
    </div>
    <?php } ?>
...
<script type="text/javascript">
	$(document).ready(function() {
		$(".page-link").click(function(){
			...
			<?php }?>
			$.ajax({
				url: "<?php echo SITEPATH;?>half-survey-result-by-user-pagination.php",
				type: "GET",
				data: {
