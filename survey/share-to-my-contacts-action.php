            ...
            while(($line = fgetcsv($csvFile)) !== FALSE){
                ...
                if($scheduleday=="Send Now")
                {
                    ...
                    if($sent_reciept){
                       ...
                       $_SESSION['error'] = "<strong>Sorry</strong> Invalid File or Blank File.";
                       header("Location:share-to-my-contacts?surveyid=".$_POST['surveyid']);
                    }
                }
                if($scheduleday=="Schedule Later")
                {
                    ...
                    $sharetempdata = $obj_survey->insertShareTempData($insert_dataa, 0);
                    if($sharetempdata){
                ...
                if($rec_name != "" && $email != "")
                    {
                        ...
                        $insert_datar['name'] = mysqli_real_escape_string($conn, $line[0]);
                        $insert_datar['email'] = mysqli_real_escape_string($conn, $line[1]);
                        $insert_datar['survey_id'] = mysqli_real_escape_string($conn, $_POST['surveyid']);
                        $insert_datar['created_at'] = date("Y-m-d H:i:s");
                        $obj_survey->insertShareRecTempData($insert_datar, 0);
                    }
            ...
            if($scheduleday=="Schedule Later")
            {
                ...
                if($sharetempdata)
                {
                    if($rec_name != "" && $email != "")
                    {
                        ...
                        $insert_datar['name'] = mysqli_real_escape_string($conn, $rec_name);
                        $insert_datar['email'] = mysqli_real_escape_string($conn, $email);
                        $insert_datar['survey_id'] = mysqli_real_escape_string($conn, $_POST['surveyid']);
                        $insert_datar['created_at'] = date("Y-m-d H:i:s");
                        $obj_survey->insertShareRecTempData($insert_datar, 0);
                    }
