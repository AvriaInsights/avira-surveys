...
<body>
    ...
    <!---left-pattern-start-->
        <div class="left-side-pattern">
            <img src="<?php echo SITEPATHFRONT;?>images/left-pattern-all.png" class="img-fluid" alt="left-side-pattern">
        </div>
    <!--left-pattern-end-->
            ...
            <div class="col-lg-8 offset-lg-2 col-md-10 offset-md-1 align-middle-content-1">
                ...
                <div class="option-holder que_section" id="divQstn">
                    ...
                    <div id="que_<?php echo $srno;?>" style="display:none;" class="que_scroll item" data-id='<?php echo $srno;?>'  isAttempted="0">
                            ...
                            <div class="row space-all">
                                <div class="col-md-12">
                                    <div class="row order-type">
                                            ...
                                            <!--Left side div-->
                                            <div class="col-md-5 border pt-4 sortable-list cust-col" id="list_<?php echo $all_question['question_id'];?>">
                                                <?php $counter=0; foreach($all_subpoints as $all_subpoint_seq){ $counter++?>
                                                <label class="item r-item checkbox-item label-icon-rank" for="order_<?php echo $all_subpoint_seq['question_subid'];?>" id="order_<?php echo $all_subpoint_seq['question_subid'];?>" name="order_<?php echo $all_subpoint_seq['question_subid'];?>" style="cursor:move;">
                                            ...
                                            <div class="col-md-5 border pt-4 sortable-list cust-col sorted-data" id="list1_<?php echo $all_question['question_id'];?>">
                                                <div class="star fs-5" id="rankorder_error_message_<?php echo $all_question['question_id'];?>"></div>
                                                <label class="r-item checkbox-item label-icon-rank" style="opacity: 0.5;" id="dummylist_<?php echo $all_question['question_id'];?>">
                                                    <span>Drag & Drop Left Side Options For Ranking</span>
                                                    <span class="move-icon">:::</span>
                                                </label>
                                            </div>
                                            ...
                                            <input type="hidden" id="rank_order_total_count_<?php echo $all_question['question_id'];?>" name="rank_order_total_count_<?php echo $all_question['question_id'];?>" value="<?php echo count($all_subpoints); ?>">
                        ...
                        <div class="row pt-6">
                            <div class="col-md-12">
                                <a class="btn save-nxt-btn action-btn-disabled" id="button_next_order_<?php echo $all_question['question_id'];?>" onclick="SetAttempted(this,<?php echo $all_question['sequence'];?>,<?php echo $all_question['question_id'];?>,<?php echo $srno;?>);chkrankset('<?php echo $srno;?>','<?php echo $all_question['question_id'];?>');setnext(<?php echo $srno;?>,'',<?php echo $all_question['question_id'];?>,'');">Save & Next
                                       <i class="fa fa-angle-right ps-3"></i>
                                </a>
                                <a id="next" class="next btn_skip skip-txt" title="Skip" onclick="SetAttempted(this,<?php echo $all_question['sequence'];?>,<?php echo $all_question['question_id'];?>,<?php echo $srno;?>)">Skip</a>
                            </div>
                        </div>
...
$('body').on('click', '.next', function() {
    ...
    ///////////////////For rank order skip remove question and answer///////////////////////////
     var qtypenamecurr=$("#qtypename_"+id).val();
    //alert(qtypename);
    if(qtypenamecurr=="Order")
    {
      var qqid=$("#qidthis_"+id).val();
      var ranksubval=$("#allsubval_"+qqid).val();
      //alert(ranksubval);
      if(ranksubval=="")
      {
            ...
            var answ123 = $("#all_answers").val();
            var setansarray = answ123.split("♣");
            ...
            if(posq!="-1")
            {
              setquestionarray.splice(posq, 1);
              var chktans12 = setquestionarray+'';
              var test2211 = chktans12.replace(/,/g,"♣");
              $("#all_question_id").val(test2211);
              
              setansarray.splice(posq,1);
              var chktans123 = setansarray+'';
              var test221133 = chktans123.replace(/,/g,"♣");
              $("#all_answers").val(test221133);
            }
...
{    //alert(curqno);
   ...
   //alert(nextqid);
    ...
    if($("#all_question_id").val()=="")
    {
         ...
         {
             var selval123=$("#allsubval_"+qid).val();
             if(selval123!="")
             {
                 $("#all_question_id").val(qid);
                 var posq="-1"; 
...
function setorderval(currsrno)
{
      ...
      var rankqid=$("#qidthis_"+currsrno).val();
      
      
      $("#list1_"+lstqstid).sortable({ opacity: 0.8, cursor: 'move',animation: 150, update: function() {
        ...
        {
          var order = 'order=&update=update&qstid='+qstid;  
        }
        ...
        $.post("<?php echo SITEPATHFRONT;?>update-subquestion-sequence.php", order, function(theResponse){
                //alert(theResponse);
                var ranktin = theResponse;
                
                if(theResponse=="")
                {
                    
                    $("#rankorder_error_message_"+rankqid).text('Please select sequence');
                    $("#allsubval_"+qstid).val(theResponse);
                    $("#dummylist_"+qstid).show();
                    $("#dummylistdown_"+qstid).remove();
                    $('#button_next_order_'+qstid).addClass("action-btn-disabled");
                    
                    
                }
                else
                {
                    $("#dummylist_"+qstid).hide();
                    $("#allsubval_"+qstid).val(theResponse);
                    $("#rankorder_error_message_"+qstid).text('');
                    $('#button_next_order_'+qstid).removeClass("action-btn-disabled");
                    
                    
                    
                    if(ranktin!="")
                    {
                          var ccnt=ranktin.split("♦");
                          var arrlen = ccnt.length;
                         
                          
                          var subtotalcnt=$("#rank_order_total_count_"+qstid).val();
                          if(arrlen==1)
                          {
                              
                              $("#dummylistdown_"+qstid).remove();
                              $("#list1_"+qstid).append('<label class="r-item checkbox-item label-icon-rank" style="opacity: 0.5;" id="dummylistdown_'+qstid+'"><span>Drag & Drop Left Side Options For Ranking</span><span class="move-icon">:::</span></label>');
                             
                          }
                          if(subtotalcnt==arrlen)
                          { 
                              $("#dummylistdown_"+qstid).hide();
                          }
                          if(subtotalcnt>arrlen)
                          {
                              $("#dummylistdown_"+qstid).show();
...
$(function() {
	...
	$('.sortable-list').sortable({
    	...
    	og = $(this).closest('.sortable-list');
		},
		over: function(event,ui) {
			n = $(this).closest('.sortable-list');
...
function chkrankset(srno,qid)
{
    var rankval=$("#allsubval_"+qid).val();
    if(rankval=="")
    {
        $("#rankorder_error_message_"+qid).text('Please select sequence');
