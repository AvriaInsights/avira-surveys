        ...
        if(is_uploaded_file($_FILES['file']['tmp_name'])){
            ...
            while(($line = fgetcsv($csvFile)) !== FALSE){
             ...
             //get category_id
              ...
              $keytrends = trim($line[0]);
                    $category_detail = $obj_category->getSingleNewsDetail('bundle_name', "bundle_name = '" . $keytrends . "'", '', 1, 0);
                ...
                $insert_data['impact'] = mysqli_real_escape_string($conn, $impact);
                $insert_data['key_trend'] = mysqli_real_escape_string($conn, $key_trend);
                $insert_data['direction'] = mysqli_real_escape_string($conn, $direction);
                $insert_data['industries_impact'] = mysqli_real_escape_string($conn, $industries_impact);
                $insert_data['created_at'] = date("Y-m-d h:i:s");
                $insert_data['updated_at'] = date("Y-m-d h:i:s");
                
                $obj_category->insertKeyReport($insert_data, 0);
            ...
            $qstring = '?status=succ';
            echo "<script> alert('Keytrend Record Added succesfully....'); </script>";
        }else{
            $qstring = '?status=err';
        }
    }else{
